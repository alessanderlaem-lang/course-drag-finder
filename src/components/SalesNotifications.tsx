...
// Force browser to fetch & decode the image immediately at module load time
...
const SalesNotifications = () => {
  ...
  const [items, setItems] = useState<NotifItem[]>([
    { uid: 0, price: NOTIFICATIONS[0] },
    { uid: 1, price: NOTIFICATIONS[1] },
    { uid: 2, price: NOTIFICATIONS[2] },
  ]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setItems((prev) => {
        ...
        nextNI.current++;
        const newItem: NotifItem = { uid: nextUid.current++, price: NOTIFICATIONS[ni] };
        ...
        <div className="relative flex justify-center">
          <AnimatePresence initial={false}>
            {items.map((item, position) => (
              <m.div
                key={item.uid}
                initial={item.uid >= 3 ? { y: -70, scale: SCALES[0] } : false}
                animate={{ y: Y_POSITIONS[position], scale: SCALES[position] }}
                exit={{ y: 310, scale: 0.75, opacity: 0 }}
              ...
              >
                {/* Use CSS background-image — avoids any decode/paint delay */}
                <div
                  className="w-12 h-12 md:w-14 md:h-14 rounded-xl flex-shrink-0"
                  style={{
                    backgroundImage: `url(${riseLogo})`,
